...
public class Packers_MoversSteps {
    ...
    public void click_within_city() {
		  ...
		  boolean clicked = moversPage.clickWithinCity(); Assert.assertTrue(clicked,
		  "Within City not clicked"); Base.sleep(1000);
    ...
    public void navigate_inventory_page() {
    	boolean clicked = moversPage.navigateToInventoryWithoutAdding();
    	boolean close=moversPage.removeItemThatAdded();
        Assert.assertTrue(clicked, "without adding the item and click continue");
        Base.sleep(1000);
    }
    ...
    public void verify_alert_message() {
        boolean alert = moversPage.isAlertPresent();
        Assert.assertTrue(alert, "Expected an alert but none displayed!");

        if (alert) {
            String msg = moversPage.getAlertMessage();
            System.out.println("Alert text: " + msg);
            // You can also add Assert here for expected text
            Assert.assertEquals(msg.trim(), "Please add items to calculate your quotation");
        }
    }

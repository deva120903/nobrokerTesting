...
public class Painting_CleaningSteps {
    ...
    public void click_home_cleaning() {
        ...
        Assert.assertTrue(clicked, "Home Cleaning not clicked");
        Base.sleep(1000); // wait for submenu to expand
    }
    ...
    public void add_all_available_items() {
        boolean added = paintingPage.addAllItems();
        Assert.assertTrue(added, "Failed to add items");
        Base.sleep(1000); // wait for add animation/modal update
    }
    ...
    public void verify_all_items_added() {
        ...
        Assert.assertTrue(verified, "Items not added correctly");
        Base.sleep(500);
    }

    @When("User removes some items")
    public void remove_some_items() {
        boolean removed = paintingPage.removeSomeItems();
        Assert.assertTrue(removed, "Failed to remove items");
        Base.sleep(1000); // wait for remove animation/modal update
    }
    ...
    public void verify_removed_items() {
        ...
        Assert.assertTrue(verified, "Items not removed correctly");
        Base.sleep(500);
    }
}
